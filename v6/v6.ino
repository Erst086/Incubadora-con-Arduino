...
#define VLT_PIN 6         // Pin donde está conectado el VLT
...
unsigned long vltActivationTime = 0; // Tiempo en que se activó el VLT
int dayCounter = 0;
int hourCounter = 0;
int minuteCounter = 0;
int fourHourCounter = 0;
bool isStarted = false;  // Estado del sistema
bool vltActive = false;  // Estado del pin VLT
...
void setup() {
  ...
  // Inicializar el pin VLT como salida
  pinMode(VLT_PIN, OUTPUT);
  digitalWrite(VLT_PIN, HIGH); // Asegurar que esté apagado inicialmente
}
...
void loop() {
  ...
  if (fourHourCounter >= 240) { // 240 minutos en 4 horas
    ...
    // Activar la salida VLT durante 10 segundos
    digitalWrite(VLT_PIN, LOW);
    vltActivationTime = currentMillis;
    vltActive = true;
  }

  if (vltActive && currentMillis - vltActivationTime >= 10000) { // 10 segundos en milisegundos
    digitalWrite(VLT_PIN, HIGH);
    vltActive = false;
  }
